...
interface FileItem {
    id: string;
    name: string;
    type: 'drive' | 'folder' | 'file' | 'app';
    icon?: string;
}

const fileSystem: Record<string, FileItem[]> = {
    root: [
        { id: 'c_drive', name: 'Local Disk (C:)', type: 'drive', icon: myComputerIcon },
        { id: 'd_drive', name: 'CD Drive (D:)', type: 'drive', icon: myComputerIcon }, // Placeholder
        { id: 'control_panel', name: 'Control Panel', type: 'app', icon: controlPanelIcon },
    ],
    c_drive: [
        { id: 'program_files', name: 'Program Files', type: 'folder' },
        { id: 'windows', name: 'WINDOWS', type: 'folder' },
        { id: 'documents_and_settings', name: 'Documents and Settings', type: 'folder' },
    ],
    d_drive: [],
    program_files: [],
    windows: [],
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', fontFamily: 'Tahoma, sans-serif' }}>
            ...
            <div style={{
                ...
                <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: '5px', backgroundColor: 'white', border: '1px solid #7F9DB9', padding: '2px 5px', fontSize: '11px' }}>
                    <img src={myComputerIcon} style={{ width: '16px', height: '16px' }} />
                    <span>{currentPath === 'root' ? 'My Computer' : fileSystem['root'].find(i => i.id === currentPath)?.name || currentPath}</span>
                </div>
            </div>
                ...
                <div style={{
                    ...
                    <div style={{ backgroundColor: 'white', borderRadius: '3px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '10px', fontSize: '11px', color: '#215DC6', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            <div style={{ cursor: 'pointer', display: 'flex', gap: '5px' }}>View system information</div>
                            <div style={{ cursor: 'pointer', display: 'flex', gap: '5px' }}>Add or remove programs</div>
                            <div style={{ cursor: 'pointer', display: 'flex', gap: '5px' }}>Change a setting</div>
                        </div>
                    </div>
                    ...
                    <div style={{ backgroundColor: 'white', borderRadius: '3px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '10px', fontSize: '11px', color: '#215DC6', display: 'flex', flexDirection: 'column', gap: '5px' }}>
                            ...
                            <div style={{ cursor: 'pointer' }}>Control Panel</div>
                        </div>
                    </div>
                    ...
                    <div style={{ backgroundColor: 'white', borderRadius: '3px', overflow: 'hidden' }}>
                        ...
                        <div style={{ padding: '10px', fontSize: '11px', color: 'black' }}>
                            <div style={{ fontWeight: 'bold' }}>{selectedItem ? selectedItem : 'My Computer'}</div>
                            <div>{selectedItem ? 'System Folder' : 'System Folder'}</div>
                        </div>
                    </div>
                </div>
